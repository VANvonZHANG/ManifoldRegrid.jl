...
Global conservation is not a property of this normalization — it follows from
nothing being renormalized: the constructor's column-sum invariant
`Σ_d W[d, s] ≈ A_src` carries each source area through the matrix untouched, so
the integral survives to machine precision even when the overlap areas carry
rounding error. Dividing by the exact destination cell area (rather than by the
row sum of `W`) is what governs how *row*-sum error propagates into local
values: geometric error degrades local accuracy only.
